...
TimeObjectUtil = {
     ...
     */
    formatterDateTimeNotss : function(date) {
        var datetime = date.getFullYear()
            + "-"// "年"
            + ((date.getMonth() + 1) >= 10 ? (date.getMonth() + 1) : "0"
            + (date.getMonth() + 1))
    ...
    /**
     * 获取最近几天[开始时间和结束时间值,时间往前推算] 没有时分秒
     */
    getRecentDaysDate : function(day) {
        var daymsTime = day * 24 * 60 * 60 * 1000;
        var yesterDatsmsTime = this.getCurrentMsTime() - daymsTime;
        var startTime = this.longMsTimeConvertToDateTime(yesterDatsmsTime);
        var pastDate = this.formatterDate(new Date(startTime));
        var nowDate = this.formatterDate(new Date());
        var obj = {
            startTime : pastDate,
            endTime : nowDate
        };
