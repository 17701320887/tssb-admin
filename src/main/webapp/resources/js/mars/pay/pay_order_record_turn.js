            ...
            if(ExceptionDialog(data)){
                ...
                if(data.type == 0){//普通商品
                    ...
                    $(data.commodities).each(function(index, com){
                        ...
                        coms.push('<input value="'+com.classNo+'" data-id="'+com.id+'" data-price="'+com.realPrice+'" name="classNos" type="checkbox" required');
                        if(checkbox == 1) {//套餐
                    ...
                    if(checkbox == 1){//套餐
                        ...
                        if(data.commoditySpecial.isDiscount!=2){
                            ...
                            for (var i = 0; i < data.commodities.length - 1; i++) {
                                coms.push('<input type="hidden" id="comSp'+(i+2)+'" value="'+obj[i+2]+'" />123');
                            }
                        }
        ...
        $.post(path+'/payDiscount/code', {
            ...
            if(ExceptionDialog(data)) {
                if(isNaN(data.result)){
                    ...
                    $('#moneyMsg').html((balance?'减：余额：'+balance+'元<br>':'')+'减：优惠券：'+discount.discount+'元');
